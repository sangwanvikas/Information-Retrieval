...
# 3. Train 'linear_model' Linear Regression Model from sklearn library.
...
class LinearRegressionTrainer:
    ...
    def __init__(self, featureFilePath):
        ...
        # print iris.data
        # print iris.target
        self._qIdsForTraining = Resource.QIdsForTrainingLR
        self._qIdsForTesting = Resource.QIdsForTestingLR
    ...
    def __FeedMLAlgo__(self):
        ...
        lr_model = self.__Train__(array(trainedSamples), array(label))

        # -----------------------------------------------------------------------------------------------------------
        # Predict and Save Trec-Result file for TrainedData
        self.__CalculateAndStorePredictedTrecResult__(Resource.SortedTrainedTrecResultFilePathLR, self._qIdsForTraining,
                                          qIdDocIds, lr_model, trainedSamples)

        print "Queries for Training Data -> ", self._qIdsForTraining
        print "Treac-Result for Training Queires saved in -> ", Resource.SortedTrainedTrecResultFilePathLR
        print "\n"

        # -----------------------------------------------------------------------------------------------------------
        # Predict and Save Trec-Result file for TrainedData
        testingSamples, testingQIdDocIds = self.__GetSamplesForTrainingQueryId__(self._feature_Matrix)
        self.__CalculateAndStorePredictedTrecResult__(Resource.SortedTestingTrecResultFilePathLR, self._qIdsForTesting,
                                                      testingQIdDocIds, lr_model, testingSamples)
        print "Queries for Testing Data -> ", self._qIdsForTesting
        print "Treac-Result for Testing Queires saved in -> ", Resource.SortedTestingTrecResultFilePathLR
        ...
        with open(filePath, "w") as handle:
            for qId, docIdToScore in qIdToDocIdToScoreDict.iteritems():
                ...
                for key, value in data:
                    ...
                    handle.write(aLine)
# ###########################################################
trainerObj = LinearRegressionTrainer(Resource.FeatureFilePath)
